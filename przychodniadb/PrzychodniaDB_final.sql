...
/****** Object:  Table [dbo].[DoctorSpecializations]    Script Date: 26.05.2026 ******/ 
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
INSERT [dbo].[DoctorSpecializations] ([DoctorID], [SpecializationID], [IsPrimary]) VALUES (2, 6, 1)
GO
...
/****** Object:  Index [IX_DoctorSpecializations_Doctor_IsPrimary]    Script Date: 26.05.2026 ******/
CREATE NONCLUSTERED INDEX [IX_DoctorSpecializations_Doctor_IsPrimary]
ON [dbo].[DoctorSpecializations] ([DoctorID], [IsPrimary])
GO
...
ALTER TABLE [dbo].[DoctorSpecializations] ADD  CONSTRAINT [DF_DoctorSpecializations_IsPrimary] DEFAULT ((0)) FOR [IsPrimary]
GO
...
ALTER TABLE [dbo].[DoctorSpecializations]  WITH CHECK ADD  CONSTRAINT [FK_DoctorSpecializations_Doctors] FOREIGN KEY([DoctorID])
REFERENCES [dbo].[Doctors] ([DoctorID])
GO
ALTER TABLE [dbo].[DoctorSpecializations] CHECK CONSTRAINT [FK_DoctorSpecializations_Doctors]
GO

ALTER TABLE [dbo].[DoctorSpecializations]  WITH CHECK ADD  CONSTRAINT [FK_DoctorSpecializations_Specializations] FOREIGN KEY([SpecializationID])
REFERENCES [dbo].[Specializations] ([SpecializationID])
GO
ALTER TABLE [dbo].[DoctorSpecializations] CHECK CONSTRAINT [FK_DoctorSpecializations_Specializations]
GO
...
/****** Object:  Trigger [dbo].[TR_Doctors_AssignLekarzRole]    Script Date: 26.05.2026 ******/
CREATE TRIGGER [dbo].[TR_Doctors_AssignLekarzRole]
ON [dbo].[Doctors]
AFTER INSERT
AS
BEGIN
	...
	SELECT DISTINCT i.UserID, r.RoleID
	FROM inserted i
	INNER JOIN [dbo].[Roles] r ON r.RoleName = 'Lekarz'
	WHERE NOT EXISTS (
		SELECT 1
...
/****** Object:  Trigger [dbo].[TR_Patients_AssignPacjentRole]    Script Date: 26.05.2026 ******/
CREATE TRIGGER [dbo].[TR_Patients_AssignPacjentRole]
ON [dbo].[Patients]
AFTER INSERT
AS
BEGIN
	...
	SELECT DISTINCT i.UserID, r.RoleID
	FROM inserted i
	INNER JOIN [dbo].[Roles] r ON r.RoleName = 'Pacjent'
	WHERE NOT EXISTS (
		SELECT 1
		FROM [dbo].[UserRoles] ur
		WHERE ur.UserID = i.UserID
...
/****** Object:  Trigger [dbo].[TR_Doctors_SyncPrimarySpecialization]    Script Date: 26.05.2026 ******/
CREATE TRIGGER [dbo].[TR_Doctors_SyncPrimarySpecialization]
ON [dbo].[Doctors]
AFTER INSERT, UPDATE
AS
BEGIN
	...
	UPDATE ds
	SET IsPrimary = 0
	FROM [dbo].[DoctorSpecializations] ds
	INNER JOIN inserted i ON i.DoctorID = ds.DoctorID
	WHERE ds.SpecializationID <> i.SpecializationID
	  AND ds.IsPrimary = 1;

	MERGE [dbo].[DoctorSpecializations] AS target
	USING (
		SELECT DoctorID, SpecializationID
		FROM inserted
	) AS source
	ON target.DoctorID = source.DoctorID
	   AND target.SpecializationID = source.SpecializationID
	WHEN MATCHED THEN
		UPDATE SET IsPrimary = 1
	WHEN NOT MATCHED THEN
		INSERT (DoctorID, SpecializationID, IsPrimary)
		VALUES (source.DoctorID, source.SpecializationID, 1);
END
GO

/****** Object:  Trigger [dbo].[TR_DoctorSpecializations_UpdateDoctorPrimary]    Script Date: 26.05.2026 ******/
CREATE TRIGGER [dbo].[TR_DoctorSpecializations_UpdateDoctorPrimary]
ON [dbo].[DoctorSpecializations]
AFTER INSERT, UPDATE
AS
BEGIN
	SET NOCOUNT ON;
	...
	INNER JOIN inserted i ON i.DoctorID = ds.DoctorID
	WHERE i.IsPrimary = 1
	  AND ds.SpecializationID <> i.SpecializationID
	  AND ds.IsPrimary = 1;

	UPDATE d
	SET SpecializationID = i.SpecializationID
	FROM [dbo].[Doctors] d
	INNER JOIN inserted i ON i.DoctorID = d.DoctorID
	WHERE i.IsPrimary = 1
	  AND d.SpecializationID <> i.SpecializationID;
END
GO
